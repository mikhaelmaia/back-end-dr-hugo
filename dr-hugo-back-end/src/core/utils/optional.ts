...
export class Optional<T> {
  ...
  private constructor(value?: T) {
    if (nonNull(value)) this.value = value as T;
  }
  ...
  public or(other: Optional<T>): Optional<T> {
    ...
    return isNull(other) ? Optional.EMPTY : other;
  }
  ...
  public orElseThrow(supplier: () => Error): T {
    if (this.isEmpty()) throw supplier();
    return this.value;
  }
  ...
  public ifPresent(consumer: (value: T) => void): void {
    if (this.isPresent() && nonNull(consumer)) consumer(this.value);
  }
  ...
  public ifPresentOrElse(
    ...
    if (this.isPresent()) nonNull(consumer) && consumer(this.value);
    else nonNull(emptyAction) && emptyAction();
  }
}
