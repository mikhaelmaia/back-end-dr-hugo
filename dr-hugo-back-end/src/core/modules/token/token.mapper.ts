...
export class TokenMapper extends BaseMapper<Token, TokenDto> {
  public toDto(entity: Token): TokenDto {
    ...
    tokenDto.renewalTime = entity.renewalTime;
    tokenDto.expirationTime = entity.expirationTime;
    tokenDto.createdAt = entity.createdAt;
    tokenDto.updatedAt = entity.updatedAt;
    return tokenDto;
  }

  public toEntity(dto: Partial<TokenDto>): Token {
    const token = new Token();
    token.id = dto.id;
    token.token = dto.token;
    token.hash = dto.hash;
    token.type = dto.type;
    token.identification = dto.identification;
    token.renewalTime = dto.renewalTime;
    token.expirationTime = dto.expirationTime;
    token.createdAt = dto.createdAt;
    token.updatedAt = dto.updatedAt;
    return token;
  }

  public toValidationDto(entity: Token): TokenValidationDto {
