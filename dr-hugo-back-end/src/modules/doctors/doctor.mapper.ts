...
export class DoctorMapper extends BaseMapper<Doctor, DoctorDto> {
  public toDto(entity: Doctor): DoctorDto {
    ...
    if (entity.registration) {
      const registrationDto = new DoctorRegistrationDto();
  ...
  public mapValidatedToDoctor(
    doctorValidatedDto: DoctorRegistrationValidatedDto,
  ): Doctor {
    ...
    const registration =
      this.mapRegistrationToDoctorRegistrationEntity(doctorData);
    doctor.registration = registration;

    const specialties = this.mapSpecialtiesDtosToEntities(
  ...
  public mapRegistrationToDoctorRegistrationEntity(
    doctorValidatedData: DoctorRegistrationData,
  ): DoctorRegistration {
    ...
    registration.situation = findEnumValueByKeyOrValue(
      DoctorSituation,
      doctorValidatedData.situation,
    );
    registration.type = findEnumValueByKeyOrValue(
      DoctorRegistrationType,
      doctorValidatedData.type,
    );
    registration.state = findEnumValueByKeyOrValue(
      BrazilianState,
      doctorValidatedData.state,
    );

    return registration;
  }

  public mapSpecialtiesDtosToEntities(
    specialtyDtos: DoctorSpecializationDto[],
  ): DoctorSpecialization[] {
    return specialtyDtos.map((dto) => this.mapSpecialtyDtoToEntity(dto));
  }

  public mapSpecialtyDtoToEntity(
    specialtyDto: DoctorSpecializationDto,
  ): DoctorSpecialization {
    const specialty = new DoctorSpecialization();
    specialty.rqe = specialtyDto.rqe;
    specialty.name = findEnumValueByKeyOrValue(
      DoctorSpecializationType,
      specialtyDto.name,
    );
    return specialty;
