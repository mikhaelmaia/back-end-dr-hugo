...
import { IsNotEmpty, IsString, IsEmail, MaxLength, IsStrongPassword, Length, IsOptional, IsEnum, ValidateNested, ValidateIf } from "class-validator";
...
import { provideIsNotEmptyValidationMessage, provideIsStringValidationMessage, provideIsEmailValidationMessage, provideMaxLengthValidationMessage, provideIsNotEmptyStringValidationMessage, provideLengthValidationMessage, provideIsValidTaxIdValidationMessage, provideIsEnumValidationMessage } from "src/core/vo/consts/validation-messages";
...
export class CreateInstitutionDto {
    ...
    @ApiProperty({ 
        description: 'Senha de acesso da instituição médica (deve ser forte: minúscula, maiúscula, número e caractere especial)',
    ...
    @ApiProperty({
        description: 'Código do país da instituição médica (ISO 3166-1 alfa-2 ou alfa-3)',
        example: 'BRA',
        minLength: 1,
        maxLength: 3,
        type: String
    ...
    @ApiProperty({
        description: 'Código de discagem direta internacional (IDD) do país da instituição médica',
        example: '+55',
        minLength: 1,
        maxLength: 5,
        type: String
    ...
    @ApiPropertyOptional({
        description: 'Código Nacional de Estabelecimentos de Saúde (CNES) - 7 dígitos numéricos',
        example: '1234567',
        minLength: 7,
        maxLength: 7,
        type: String
    ...
    @IsNotEmpty({
        message: 'Especificação do tipo de instituição é obrigatória quando selecionado "Outro"',
    })
    @IsString({
    ...
    @ApiPropertyOptional({
        description: 'Especificação do tipo de instituição quando selecionado "Outro"',
