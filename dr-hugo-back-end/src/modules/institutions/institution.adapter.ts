...
export class InstitutionAdapter {
    ...
    public async lookupTaxId(taxId: string): Promise<InstitutionValidatedDto> {
        ...
        await this.cacheService.set(`${this.CACHE_KEY_PREFIX}${taxId}`, institutionValidated, this.VALIDATION_TTL_SECONDS);
    ...
    private isValidInstitution(situation: string): boolean {
        return this.COMPANY_VALID_SITUATIONS.includes(situation?.toLowerCase());
    }

    private mapToInstitutionValidatedDto(response: ReceitaWsResponseDto): InstitutionValidatedDto {
    ...
    private buildInstitutionValidationData(company: CompanyDto): InstitutionValidationData {
        ...
        data.mainActivities = company.atividade_principal?.map(activity => activity.text);
        data.secondaryActivities = company.atividades_secundarias?.map(activity => activity.text);
        ...
        data.situation = company.situacao;

        return data;
    }

    private buildErrorMessage(response: ReceitaWsResponseDto): InstitutionValidatedDto {
        const result = new InstitutionValidatedDto();
        result.valid = false;
        result.data = null;
        result.message = response.error?.message || 'Erro ao consultar dados da empresa';
        return result;
    }
}